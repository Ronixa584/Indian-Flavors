...
//       <div className="filters-items fixed top-0 w-full z-50 flex flex-wrap gap-4 justify-center my-4 px-4">
//         <button
...
const Body = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (window.scrollY >= 2*bannerHeight+5) {
        setIsFixed(true);
      } else {
        setIsFixed(false);
      }
  ...
  return allRestaurants.length === 0 ? (
    ...
    <div className="Body min-h-screen ">
      ...
      <div
        className={`filters-items w-full z-50 flex flex-wrap gap-4 justify-center  transition-all duration-200  ${
          isFixed
            ? "fixed top-0 bg-gradient-to-r from-green-100 via-white-800 to-red-100 w-full shadow-md"
            : ""
        }`}
      >
        <div className="flex flex-wrap gap-4 justify-center my-4 px-4">
          <button
            className="filter-btn p-2 bg-gray-200 text-gray-800 rounded-md shadow-md hover:bg-gray-300 transition duration-200"
            onClick={handleSearch}
          >
          ...
          <button
            className="filter-btn p-2 bg-red-500 text-white
rounded-md shadow-md hover
transition duration-200"
      ...
      {/* Restaurants loading */}
      <div
        className={`restaurant-card-list flex flex-wrap place-content-evenly ${
          isFixed ? "shadow-md mt-16" : ""
        }`}
      >
        {filteredRestaurants.map((restaurant) => (
          <Link
